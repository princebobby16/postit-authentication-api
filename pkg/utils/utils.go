...
func ProvisionSchema(request models.SignUpRequest, passwordHash []byte) (string, error) {
	...
	// create tables
	query = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.post(post_id uuid UNIQUE NOT NULL, post_message text NOT NULL, post_image character varying (200), hash_tags text[], post_status boolean NOT NULL, post_priority boolean NOT NULL, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (post_id)); CREATE TABLE IF NOT EXISTS %s.schedule( schedule_id uuid UNIQUE NOT NULL, schedule_title character varying(200), schedule_from timestamp with time zone NOT NULL, schedule_to timestamp with time zone NOT NULL, post_ids character varying(200)[] NOT NULL, duration_per_post float NOT NULL, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (schedule_id)); CREATE TABLE IF NOT EXISTS %s.scheduled_post(scheduled_post_id uuid NOT NULL, post_id uuid NOT NULL, post_message text NOT NULL, post_image character varying (200), hash_tags text[], post_status boolean NOT NULL, post_priority boolean NOT NULL, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP); CREATE TABLE IF NOT EXISTS %s.application_info(application_uuid uuid UNIQUE NOT NULL, application_name character varying (200) NOT NULL, application_id character varying(200) NOT NULL, application_secret character varying (200) NOT NULL, application_url character varying (200) NOT NULL, application_code character varying (200), created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(application_uuid));", tenantNamespace, tenantNamespace, tenantNamespace, tenantNamespace)
	_, err = connection.Exec(query)
	if err != nil {
		return "", err
	}
